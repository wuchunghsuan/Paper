		...
		\ForAll{$r$ in $p\_reduces$}
			...
			\If{$new\_prob\geq r.prob$}
				...
				\State \begin{varwidth}[t]{\linewidth}
					$r.host\_id$ \par
					  \hskip\algorithmicindent $\gets \text{\emph{shuffles}}\left[r.rid\right].assigned\_host$
					\end{varwidth}
			\EndIf
		\EndFor
		\State $M\gets$ $SCHEDULE\left(m, host\_id, p\_reduces\right)$\par
		\hskip\algorithmicindent
		\ForAll{$host\_id$ in $M$}
			\Comment Re-shuffle
			\ForAll{$r$ in $M\left[host\_id\right].reduces$}
				\If{$host\neq$\par
					\hskip\algorithmicindent $\text{\emph{shuffles}}\left[r.rid\right].assigned\_host$}
				\State Re-shuffle data to $host$
				\State $\text{\emph{shuffles}}\left[r.rid\right].assigned\_host$\par
					\hskip\algorithmicindent$\gets host$
				\EndIf
			\EndFor
		\EndFor
		\Return $M$

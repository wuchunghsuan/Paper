...
{\color{black}
In order to analyze above-mentioned resources scheduling strategies, we propose a performance model called \textit{Framework Resources Quantification} (FRQ) model. The FRQ model quantifies computing and I/O resources and displays resources scheduling strategies of DAG frameworks in the time dimension. We use the FRQ model to assist in analyze the deficiencies of resources scheduling and optimize it. 
...
According to the above five parameters and the scheduling strategy of the DAG framework, the FRQ model is able to calculate the execution time of each phase of a computing job. 
...
I/O resource in the map phase is not fully utilized. Furthermore, due to coupling with reduce phase, shuffle phase delays the execution time of reduce phase. 

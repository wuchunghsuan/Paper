...
The FRQ model displays utilization of resources in the time dimension and calculates execution time of computing jobs on different DAG frameworks. 
We use the FRQ model to assist in analyzing shuffle process and verify SCache shuffle optimization by mathematics.
}
...
Those evaluations have demonstrated that, by incorporating SCache, the shuffle overhead of Spark can be reduced by nearly $89\%$, and the overall completion time of TPC-DS queries improves $40\%$ on average. On Apache Hadoop, SCache optimizes end-to-end Terasort completion time by $15\%$.
}
\end{abstract}

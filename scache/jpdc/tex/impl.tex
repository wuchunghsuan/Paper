...
\edited{Due to the characteristic of shuffle data (e.g., short-lived, write-once, read-once),}
% For now, 
SCache only restarts failed workers without recovering their data \edited{and} leaves the fault handling to the DAG frameworks.
If a failure happens in a SCache worker during shuffle phases, the \edited{\texttt{getBlock}} API will return a \edited{data-not-found} error which causes the DAG framework restarts the current stage.
During the re-computing, the DAG frameworks still use SCache to gain shuffle optimization.

\edited{We believe that }
